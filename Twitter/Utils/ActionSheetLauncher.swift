...
class ActionSheetLauncher: NSObject {
    ...
    private lazy var tableViewHeight = CGFloat(viewModel.options.count * 60) + 100  // number of options * 60
    ...
    private lazy var footerView: UIView = {
        ...
        cancelButton.setHeight(height: 50)
    ...
    //MARK: - Helpers
    func show() {
        logger("Show action sheet for user \(user.username)")
...
extension ActionSheetLauncher: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let option = viewModel.options[indexPath.row].description
        cell.option = option
        return cell
    }
...
extension ActionSheetLauncher: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        perform(#selector(handleDismissal))
